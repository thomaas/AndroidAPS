...
class InvalidateGlucoseValueTransaction(val id: Long) : Transaction<Unit>() {
    override fun run() {
        val glucoseValue = AppRepository.database.glucoseValueDao.findById(id) ?: throw IllegalArgumentException("There is no such GlucoseValue with the specified ID.")
