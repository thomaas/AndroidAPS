...
class InsightHistoryTransaction(val pumpSerial: String) {
    ...
    companion object {
        internal fun process(transaction: InsightHistoryTransaction): Completable = Completable.fromCallable {
            AppRepository.database.runInTransaction {
                processBoluses(transaction.pumpSerial, transaction.boluses)
                processTemporaryBasals(transaction.pumpSerial, transaction.temporaryBasals.toMutableList())
                processTherapyEvents(transaction.pumpSerial, transaction.therapyEvents)
                processTotalDailyDoses(transaction.pumpSerial, transaction.totalDailyDoses)
                processOperatingModeChanges(transaction.pumpSerial, transaction.operatingModeChanges)
            }
        }

        private fun processTotalDailyDoses(pumpSerial: String, tdds: List<TotalDailyDose>) {
            tdds.forEach {
        ...
        private fun processTherapyEvents(pumpSerial: String, therapyEvents: List<TherapyEvent>) {
        ...
        private fun getDateStopped(pumpSerial: String, eventId: Long): Long? {
        ...
        private fun saveSuspendTBR(pumpSerial: String, startEvent: OperatingModeChange, lastStoppedEvent: OperatingModeChange?, lastPausedEvent: OperatingModeChange?) {
            val dateStopped = if (lastStoppedEvent == null) {
                getDateStopped(pumpSerial, startEvent.eventId)
            } else {
        ...
        private fun processOperatingModeChanges(pumpSerial: String, operatingModeChanges: List<OperatingModeChange>) {
                ...
                val therapyEventType = when (it.to) {
                    OperatingModeChange.OperatingMode.STARTED -> {
                        if (it.from == OperatingModeChange.OperatingMode.STOPPED) {
                            saveSuspendTBR(pumpSerial, it, lastStoppedEvent, lastPausedEvent)
                        }
        ...
        private fun updateExistingTemporaryBasal(pumpSerial: String, temporaryBasal: TemporaryBasal) {
        ...
        private fun processTemporaryBasals(pumpSerial: String, tbrs: MutableList<TemporaryBasal>) {
            tbrs.firstOrNull()?.let {
                if (!it.start) {
                    updateExistingTemporaryBasal(pumpSerial, it)
                    tbrs.removeAt(0)
                }
            }
        ...
        private fun saveStandardBolus(pumpSerial: String, start: Bolus?, end: Bolus?): Long {
        ...
        private fun saveExtendedBolus(pumpSerial: String, start: Bolus?, end: Bolus?): Long {
            val bolusId = (start?.bolusId ?: end!!.bolusId).toLong()
        ...
        private fun saveMultiwaveBolusLink(pumpSerial: String, start: Bolus?, end: Bolus?, standardBolusId: Long, extendedBolusId: Long): Long {
            val bolusId = (start?.bolusId ?: end!!.bolusId).toLong()
        ...
        private fun processBoluses(pumpSerial: String, boluses: List<Bolus>) {
            ...
            bolusPairs.forEach {
                ...
                val bolusDatabaseId = if (type == Bolus.Type.STANDARD || type == Bolus.Type.MULTIWAVE) {
                    saveStandardBolus(pumpSerial, it.first, it.second)
                } else {
                    null
                }
                val extendedBolusDatabaseId = if (type == Bolus.Type.EXTENDED || type == Bolus.Type.MULTIWAVE) {
                    saveExtendedBolus(pumpSerial, it.first, it.second)
                } else {
                    null
                }
                if (type == Bolus.Type.MULTIWAVE) {
                    saveMultiwaveBolusLink(pumpSerial, it.first, it.second, bolusDatabaseId!!, extendedBolusDatabaseId!!)
                }
            }
        }
    }
